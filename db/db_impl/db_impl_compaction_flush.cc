...
void DBImpl::MaybeScheduleFlushOrCompaction() {
  ...
  while (!is_flush_pool_empty && unscheduled_flushes_ > 0 &&
    ...
    bg_flush_scheduled_++;
    // This check is used to avoid the following race:
    // first 'if' condition is false.
    // trucate sets is_truncating_=true,
    // truncate starts running
    // bg_flush_scheduled_ is incremented,
    // new flush is scheduled, while a truncate is still running - leading to a
    // use-after-free.

    // The first if is actually not necessary, but we don't want
    // to incerement the counter just to decrement it, for robustness sake.
    if (is_truncating_) {
      bg_flush_scheduled_--;
      return;
    }
