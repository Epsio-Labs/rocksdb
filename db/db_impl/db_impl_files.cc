...
void DBImpl::PurgeObsoleteFiles(JobContext& state, bool schedule_only, bool should_lock) {
  ...
  for (const auto& candidate_file : candidate_files) {
    ...
    if (schedule_only) {
      if (should_lock) {
        LockAndSchedulePendingPurge(fname, dir_to_sync, type, number, state.job_id);
      }
      else {
...
void DBImpl::LockAndRemoveFilesGrabbedForPurge(std::unordered_set<uint64_t> files_to_del) {
...
void DBImpl::LockAndSchedulePendingPurge(std::string fname, std::string dir_to_sync,
